...
`,
	...
	for _, treeNode := range tree.entries {
		...
		builder := primitives.NewPrimitiveBuilder("example.com", tmpDir+string(os.PathSeparator)+"testdata")
...
func TestPrimitiveBuilder_AddNode_Packages(t *testing.T) {
	...
	for _, treeNode := range tree.entries {
		...
		dirOut := make(chan string)
		depVis, nodeOut := primitives.NewDependencyVisitor()
		builder := primitives.NewPrimitiveBuilder("", tmpDir)
...
`,
	...
	for _, treeNode := range tree.entries {
		...
		dirOut := make(chan string)
		depVis, nodeOut := primitives.NewDependencyVisitor()
		builder := primitives.NewPrimitiveBuilder("testdata", tmpDir+string(os.PathSeparator)+"testdata")
